...
from models.models_db import SessionLocal, FireDetection
...
def get_hotspots(
    ...
    if polygon:
        ...
        detections = [
            d
            for d in detections
            if polygon_shape.contains(Point(d.longitude, d.latitude))
        ]
...
def get_hotspot_by_id(id: int, db: Session = Depends(get_db)):
    ...
    return Feature(
        type="Feature",
        geometry=FeatGeometry(
            type="Point", coordinates=[detection.longitude, detection.latitude]
        ),
        properties={},
    )
