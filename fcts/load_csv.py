...
from models.model_db import SessionLocal, FireDetection, init_db 
...
def extract_relevant_data(row):
    ...
    lat = next((float(row[key]) for key in possible_lat_keys if key in row and row[key]), None)
    lon = next((float(row[key]) for key in possible_lon_keys if key in row and row[key]), None)

    return lat, lon

def extract_timestamps_from_filename(filename):
    """Extract timestamp_start and timestamp_end from the filename."""
    match = re.search(r"(\d{4}-\d{2}-\d{2}T\d{6}\.\d+Z)_(\d{4}-\d{2}-\d{2}T\d{6}\.\d+Z)", filename)
    if match:
